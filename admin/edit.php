...
if (isset($_POST['update'])) {    
	...
	if (empty($_FILES['berkas']['tmp_name'])) {
		...
		$result = mysqli_query($koneksi, "UPDATE printer_tb SET NamaPrinter='$NamaPrinter',SpesifikasiPrinter='$SpesifikasiPrinter',HargaPrinter='$HargaPrinter',stok='$stok' WHERE IdPrinter=$IdPrinter");
		...
		$result = mysqli_query($koneksi, "UPDATE printer_tb SET NamaPrinter='$NamaPrinter',SpesifikasiPrinter='$SpesifikasiPrinter',HargaPrinter='$HargaPrinter',image='$namaFile',stok='$stok' WHERE IdPrinter=$IdPrinter");
...
while($Printer_data = mysqli_fetch_array($result)) //Perulangan
{
    ...
    $image= $Printer_data['image']; //Image
    $stok= $Printer_data['stok']; //Stok
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Admin - Edit Printer</title>
	...
	<main class="content-wrap">
		...
		<div class="content">
            ...
            <form name="update_printer" method="post" action="edit.php" enctype="multipart/form-data">
                <table border="0">
                    <p>Preview:</p>
                    <img id="thumb" src="../img/post/<?php echo $image; ?>" width="100" height="100" />
                    <p>
                        <input type="file" id="image" name="berkas" value="<?php echo $image; ?>" onchange="preview()">
                    </p>
                    <script>
                        function preview() {
                    ...
                    <p>Nama</p>
                    <p><input type="text" name="NamaPrinter" value="<?php echo $NamaPrinter; ?>"></p>
                    
                    <p>Spesifikasi</p>
                    <p><input type="text" name="SpesifikasiPrinter" value="<?php echo $SpesifikasiPrinter; ?>"></p>
                
                    <p>Harga</p>
                    <p><input type="text" name="HargaPrinter" value="<?php echo $HargaPrinter; ?>"></p>
                
                    <p>Stok</p>
                    <p><input type="number" name="stok" value="<?php echo $stok; ?>"></p>
                
                    <p><input type="hidden" name="IdPrinter" value=<?php echo $_GET['IdPrinter'];?>></p>
